...
import 'package:admin_bite/helpers/responsiveness.dart';
...
class _ReportTableState extends State<ReportTable>
  ...
  getReportDetail() async {
    ...
    for (var element1 in restQuery.docs) {
      ...
      double commission = 0;
      commission = (earning *
              (double.parse(element1.get('commission').toString()) / 100))
          .toPrecision(2);
    ...
    for (var element in reportList) {
      setState(() {
        ...
        totalProfit = totalProfit! +
            double.parse(element['commission'].toString()).toPrecision(2);
      });
    }
    setState(() {
      loader = false;
                              ...
                              child: Row(
                                children: [
                                  Expanded(
                                    child: Center(
                                      child: Text(
                                        '\$${double.parse(totalEarning.toString()).toPrecision(2)}',
                                  ...
                                  Expanded(
                                    child: Center(
                                      child: Text(
                                        '\$${double.parse(totalProfit.toString()).toPrecision(2)}',
                                        style: const TextStyle(
                                            color: Colors.black54),
                                      ),
                                    ),
                                          ...
                                          child: Row(
                                            children: [
                                              ...
                                              Expanded(
                                                child: Center(
                                                  child: Text(
                                                    '\$${double.parse(reportList[index]['earning'].toString()).toPrecision(2)}',
                                                    style: const TextStyle(
                                                        color: Colors.black54),
                                                  ),
                                                ),
  ...
  Future<void> generateCSVPayouts() async {
    ...
    QuerySnapshot querySnapshot =
        await FirebaseFirestore.instance.collection('payouts').get();
    ...
    for (var element in querySnapshot.docs) {
      ...
      dataRow.add('\$${element.get('amount')}');
      //lastly add dataRow to our 2d list
      rows.add(dataRow);
      setState(() {
        indexPrivate = indexPrivate! + 1;
