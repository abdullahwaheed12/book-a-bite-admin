import 'package:admin_bite/pages/overview/overview.dart';
...
class _OverviewCardsLargeScreenState extends State<OverviewCardsLargeScreen> {
  ...
  getReportForProfitDetail({int days = 1}) async {
    reportListForProfit = [];
    QuerySnapshot restQuery =
        await FirebaseFirestore.instance.collection('restaurants').get();
    restQuery.docs.forEach((element1) async {
    ...
    Future.delayed(const Duration(seconds: 4))
        .whenComplete(() => getRevenueForProfit(days));
  }
  ...
  getRevenueForProfit(int days) {
    print('----------------111111-->>>>>>>>>>  ${reportListForProfit.length} ' );
                            ...
                            children: [
                              Text(
                                loader
                                    ? '...'
                                    : '\$' +
                                        double.parse(totalEarning.toString())
                                            .toPrecision(2)
                                            .toString(),
                                style: const TextStyle(
                                    fontFamily: "Nunito",
                            ...
                            children: [
                              Text(
                                loaderForProfit
                                    ? '...'
                                    : '\$' +
                                        double.parse(totalProfit.toString())
                                            .toPrecision(2)
                                            .toString(),
                                style: const TextStyle(
                                    fontFamily: "Nunito",
