import 'package:admin_bite/constants/controller.dart';
...
class BiteHubDetailsPage extends StatefulWidget {
  const BiteHubDetailsPage({Key? key, this.restModel}) : super(key: key);
  final DocumentSnapshot? restModel;

  @override
  State<BiteHubDetailsPage> createState() => _BiteHubDetailsPageState();
}

class _BiteHubDetailsPageState extends State<BiteHubDetailsPage> {
  ...
  getData() async {
    QuerySnapshot queryBags = await FirebaseFirestore.instance
        .collection('biteBags')
        .where("restaurant_id", isEqualTo: widget.restModel!.get('id'))
        .get();
    setState(() {
      totalBags = queryBags.docs.length;
    });
